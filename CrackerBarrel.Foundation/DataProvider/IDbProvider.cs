...
namespace CrackerBarrel.Foundation.DataProvider
{
    public interface IDbProvider : IDataProvider
    {
        ...
        IEnumerable<T> GetMultiMap<T, T2>(string sql, Func<T, T2, T> func, object param = null, string cacheKey = "", DateTime? lifeTime = null);
        IEnumerable<T> GetMultiMap<T, T2, T3>(string sql, Func<T, T2, T3, T> func, object param = null, string cacheKey = "", DateTime? lifeTime = null);
        int Execute(string sql, object param, string cacheKey = null, DateTime? lifeTime = null);
