...
namespace SimpleEssentials.IO.Handlers
{
    internal class FileHandler : IFileHandler
    {
        ...
        public IEnumerable<T> ReadToList<T>(IFile file, IFileReader fileReader, Dictionary<string, string> metaData = null) where T : class, new()
        {
            return fileReader?.ReadToList<T>(file.FullPath, metaData);
        }
        ...
        public IEnumerable<T> ReadBy<T>(IFile file, Func<T, bool> predicate, IFileReader fileReader, Dictionary<string, string> metaData = null) where T : class, new()
        {
            ...
            return fileReader?.ReadToList<T>(file.FullPath, metaData).Where(predicate);
        }
